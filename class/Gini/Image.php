...
class Image {
    ...
    private function & calculateSize($width, $height, $constraint=true) {
    ...
    public function resize($max_width = 0, $max_height = 0, $constraint=true) {
        ...
        list($width, $height)=$this->calculateSize($max_width, $max_height, $constraint);
    
        if(function_exists('ImageCreateTrueColor')) {
            $im = ImageCreateTrueColor($width, $height);
            ImageAlphaBlending($im, false);
            $color = $this->backgroundColor ?: ImageColorAllocateAlpha($this->im, 0, 0, 0, 127);
    ...
    public function save($format, $filename) {
        $this->show($format, $filename);
    }

    public function makeReflection($percent = 50, $opacity = 100, $border = null, $padding = 2) {
        ...
        if($border) {
            if(!is_array($border))$border = ['width'=>$border];
    ...
    function backgroundColor($color = null, $alpha = 0) {

        if ($color !== null) {
    
            if ($color === 'transparent') {
                $this->backgroundColor = ImageColorAllocateAlpha($this->im, 0, 0, 0, 127);
            }
            else {
                $rgb = $this->hex2rgb($color);
                $this->backgroundColor = ImageColorAllocateAlpha($this->im, $rgb[0], $rgb[1], $rgb[2], $alpha);
            }
    ...
    private $textColor;
    function textColor($color = null, $alpha = 0) {

        if ($color !== null) {
    
            if ($color === 'transparent') {
                $this->textColor = ImageColorAllocateAlpha($this->im, 0, 0, 0, 127);
            }
            else {
                $rgb = $this->hex2rgb($color);
                $this->textColor = ImageColorAllocateAlpha($this->im, $rgb[0], $rgb[1], $rgb[2], $alpha);
            }
    ...
    static function loadFromData($data) {
        $image = new Image;
    ...
    static function showFile($filename, $format = 'png') {
